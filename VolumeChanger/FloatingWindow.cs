...
namespace VolumeChanger
{
	...
	public class FloatingWindow : NativeWindow, IDisposable
    {
		...
		{
            using (Bitmap bitmap1 = new Bitmap(this.Size.Width, this.Size.Height, PixelFormat.Format32bppArgb))
            {
        ...
        private void AnimateWithBlend(bool show, uint time)
        {
            ...
            if (show)
            {
                ...
                double baseTime = DW.Utils.Time.GetTime();
                double currTime = baseTime;
                while (currTime - baseTime < time)
                {
                    currTime = DW.Utils.Time.GetTime();
                    double progress = (currTime - baseTime) / (double)time;
                    progress = Math.Max(progress, 0.0);
                    progress = Math.Min(progress, 1.0);

                    double alpha = originalAplha * progress;
                    if (this._alpha != Math.Round(alpha))
                    {
                        this._alpha = (byte)Math.Round(alpha);
                        this.UpdateLayeredWindow();
                    }
            ...
            else
            {
                ...
                {
                    currTime = DW.Utils.Time.GetTime();
                    double progress = 1.0 - (currTime - baseTime) / (double)time;
                    progress = Math.Max(progress, 0.0);
                    progress = Math.Min(progress, 1.0);
        ...
        private void CreateWindowOnly()
        {
            ...
            //uint ui = User32.WS_POPUP | User32.WS_CAPTION;// | User32.WS_SYSMENU | User32.WS_MINIMIZEBOX;
            uint ui = User32.WS_POPUP | User32.WS_BORDER;
            params1.Style = (int)ui;
            params1.ExStyle = User32.WS_EX_TOPMOST | User32.WS_EX_LAYERED | User32.WS_EX_NOACTIVATE;
        ...
        private void PerformWmPaint_WmPrintClient(ref Message m, bool isPaintMessage)
		{
			...
			{
                using (Bitmap bitmap1 = new Bitmap(rectangle1.Width, rectangle1.Height))
                {
